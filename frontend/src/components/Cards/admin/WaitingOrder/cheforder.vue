            ...
            <template slot-scope="{row,index}">
              <tr :style="row.isflaged==1?'background:tomato !important;color:black;':'color:black;'">
                ...
                <td style="vertical-align: middle">
                  <button class="btn btn-sm btn-success" type='button'   @click='DoneOrder(row.id,index)'>Done</button>
                </td>
              </tr>
            </template>
          </datatable>
            ...
            >
              <template slot-scope="{row,index}">
                <tr :style="row.isflaged==1?'background:tomato !important;color:black;':'color:black;'">
                  ...
                  <td style="vertical-align: middle">
                    <button class="btn btn-sm btn-success" type='button'   @click='DoneOrderAll(row.folder_id,index)'>Done</button>
                  </td>
                </tr>
              </template>
            </datatable>
  ...
  methods: {
    ...
    DoneOrderAll(id,index){
      window.Swal.fire({
            ...
            if (result.value) {
              this.$store.dispatch('admin/AdminController',{'data':{},'method':'get','api':'admin_done_order_all','variable':'status','params':id}, { root: true }).then(() => {
