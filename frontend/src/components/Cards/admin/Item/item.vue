   ...
   <div>
    ...
    border-bottom-left-radius: 10px;"></i>
                                  ...
                                  @keyup="FilterItem"
                                    ...
                                    name="search"
                                    placeholder="search any item group ..."
          ...
          >
            <template slot-scope="{row,index}">
              <tr>
                ...
                <td style="vertical-align: middle">{{ row.unit?row.unit:'' }}</td>
                <td style="vertical-align: middle">{{ row.category.title }}</td>
                ...
                <td>
                  <b-dropdown>
                    ...
                    <b-dropdown-item>
                  <span type='button'  style="width:100%;background:transparent;color:black;border:0;text-aling:left;" v-b-modal="'post-update-modal-item'" @click='Edititem(row,index)'><i class="fa fa-edit" style="color:grey;margin-right:10px;"></i>Edit</span>
                    </b-dropdown-item>
      ...
      >
        <div class="form-group row">
          ...
          required
            ...
            placeholder="title"
            class="form-control"
            :class="{ 'is-invalid': item.errors.has('title') }"
          />
          <has-error :form="item" field="title"></has-error>
        </div>
        <div class="form-group row">
          <label>Unit</label>
          <input
          required
            ...
            placeholder="unit"
            class="form-control"
            :class="{ 'is-invalid': item.errors.has('unit') }"
          />
          <has-error :form="item" field="unit"></has-error>
        </div>
        <div class="form-group row">
          ...
          required
            ...
            class="form-control"
            :class="{ 'is-invalid': item.errors.has('category') }"
          >
          <option value="">Select Category</option>
          <option v-for='category in categorys' :value="category.id" :key='category.id'>{{category.title}}</option>
          </select>
          <has-error :form="item" field="category"></has-error>
        </div>
...
export default {
  ...
  methods: {
    FilterItem(){
       this.$store.dispatch('admin/AdminController',{'data':{search:this.form.search},'method':'post','api':'admin_search_item','variable':'item'}, { root: true }).then(() => {
        this.items=this.$store.getters['admin/Getitem']
      })
    },
     getmycategory() {
    ...
    getmyitem(page=1) {
      this.$store.dispatch('admin/AdminController',{'data':{},'method':'get','api':'admin_get_pagin_item','variable':'item','params':'?page='+page}, { root: true }).then(() => {
        this.items=this.$store.getters['admin/Getitem']
      })
    },
      disable(id,index) {
        var api='admin_disable_item'
      if(this.$store.getters.role=='manager'){
         api='users_disable_item'
      }
      window.Swal.fire({
            title: 'are you sure',
      ...
      enable(id,index) {
        var api='admin_enable_item'
      if(this.$store.getters.role=='manager'){
         api='users_enable_item'
      }
      window.Swal.fire({
            title: 'are you sure',
    ...
    deleteitem(id,index) {
      window.Swal.fire({
            ...
            if (result.value) {
              this.$store.dispatch('admin/AdminController',{'data':{},'method':'delete','api':'admin_delete_item','variable':'status','params':id}, { root: true }).then(() => {
         window.Toast.fire({
            icon: "success",
    ...
    update() {
     ...
     var form =window.VFToF(this.item);
      this.$store
        .dispatch('admin/AdminController',{'data':form,'method':'put','api':'admin_update_item','variable':'item','params':id}, { root: true })
        .then(() => {
          var data = this.$store.getters["admin/Getitem"];
          this.items.data[this.updateindex].id=data.id
          this.items.data[this.updateindex].unit=data.unit
          this.items.data[this.updateindex].title=data.title
           this.items.data[this.updateindex].category.title=data.category.title
    ...
    create() {
      var form =window.VFToF(this.item);
      this.$store.dispatch('admin/AdminController',{'data':form,'method':'post','api':'admin_create_item','variable':'item'}, { root: true }).then(() => {
