<template>
   <div>
    <b-row>
      <b-col md="11" style="margin-left:2%;margin-top:30px;">
        ...
        <div class="panel">
          <div style="padding:20px;">
            ...
            <export-excel :data="json_data" :fields="json_fields" workseet="My worksheet" name='ItemsLogReport.xls' type="csv" class="btn" id="export-log-to-xl" v-show="false">
     </export-excel>
            </div>
             </div>
          ...
          >
            <template slot-scope="{row,index}">
              <tr :style='row.sold==row.amount?"background:tomato;color:black;":""' >
                ...
                <td style="vertical-align: middle"><span class="badge badge-pill btn-primary" v-if='$store.getters.getUserID==row.branch_data.user'>{{ row.branch_data.title }}</span><span class="badge badge-pill badge-dark" v-if='$store.getters.getUserID!=row.branch_data.user'>{{ row.branch_data.title }}</span> </td>
            ...
            <div  v-if="itemid==''&&items.currentPage">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { label: "Branch", field: "branch.title" },
  ...
  methods: {
      exportex(){
      this.loading=true
      this.$store.dispatch('admin/AdminController',{'data':{},'method':'get','api':'admin_get_all_items_log','variable':'items'}, { root: true }).then(() => {
    ...
    filterByItem(page=1){
       var api='admin_filter_items_log'
      if(this.$store.getters.role=='manager'){
         api='users_filter_items_log'
      }
    ...
    getmyitem(page=1) {
      var api='admin_get_items_log'
      if(this.$store.getters.role=='manager'){
         api='users_get_items_log'
      }
