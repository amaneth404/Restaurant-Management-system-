   ...
   <div>
    ...
    border-bottom-left-radius: 10px;"></i>
              ...
              <b-button
                ...
                style="padding:0;"
                v-b-modal="'post-update-modal-bank'"
                @click="resetform()"
                >
                <div class="top-1"></div>
              ...
              <tr>
                ...
                <td style="vertical-align: middle">
                  <b-dropdown>
                    ...
                    <b-dropdown-item>
                  <span type='button'  style="width:100%;background:transparent;color:black;border:0;text-aling:left;" v-b-modal="'post-update-modal-bank'" @click='Editbank(row,index)'><i class="fa fa-edit" style="color:grey;margin-right:10px;"></i>Edit</span>
                    </b-dropdown-item>
                    <b-dropdown-item>
                  <span type='button'  style="width:100%;background:transparent;color:black;border:0;text-aling:left;"  @click='deletebank(row.id,index)'><i class="fas fa-edit-alt" style="color:grey;margin-right:10px;"></i> Delete</span>
                    </b-dropdown-item>
                  </b-dropdown>
                </td>
              </tr>
    ...
    <b-modal :id="'post-update-modal-bank'" :size="'md'">
          <div style="padding-left:20px;padding-top:15px;padding-bottom:5px;border-bottom:1px solid rgba(214,214,214);font-size:1.3em;">{{display?'Update Bank':'Add Bank'}} <span><i class="fas fa-times" style="float:right;margin-right:20px;margin-top:5px;color:grey;" @click="closemoadl('post-update-modal-bank')"></i></span></div>
      <form
      ...
      >
        <div class="form-group row">
          ...
          required
            ...
            placeholder="name"
            class="form-control"
            :class="{ 'is-invalid': bank.errors.has('name') }"
          />
          <has-error :form="bank" field="name"></has-error>
        </div>
...
export default {
  ...
  methods: {
    FilterBank(){
       this.$store.dispatch('admin/AdminController',{'data':{search:this.form.search},'method':'post','api':'admin_search_bank','variable':'bank'}, { root: true }).then(() => {
        this.banks=this.$store.getters['admin/Getbank']
      })
    },
    closemoadl(val){
    ...
    deletebank(id,index) {
      window.Swal.fire({
            ...
            if (result.value) {
              this.$store.dispatch('admin/AdminController',{'data':{},'method':'delete','api':'admin_delete_bank','variable':'status','params':id}, { root: true }).then(() => {
         window.Toast.fire({
            icon: "success",
            title:'bank deleted successfuly',
          });
        this.banks.splice(index,1)
      }).catch(()=>{
         window.Toast.fire({
            icon: "error",
            title:"bank can't be deleted !!",
          });
      })
            }
     });
    ...
    update() {
     var form =window.VFToF(this.bank);

      this.$store
        .dispatch('admin/AdminController',{'data':form,'method':'put','api':'admin_update_bank','variable':'bank','params':this.bank.id}, { root: true })
        .then(() => {
          var data = this.$store.getters["admin/Getbank"];
          this.banks.data[this.updateindex].id=data.id
          this.banks.data[this.updateindex].name=data.name
          this.banks.data[this.updateindex].money=data.money
          this.banks.data[this.updateindex].created_at=data.created_at
          this.display = false;
          this.$bvModal.hide("post-update-modal-bank");
           window.Toast.fire({
            icon: "success",
            title:'bank updated successfuly',
          });
        })
        .catch((err) => {
          this.bank.errors.errors = err;
        });
    },
    create() {
      var form =window.VFToF(this.bank);
      this.$store.dispatch('admin/AdminController',{'data':form,'method':'post','api':'admin_create_bank','variable':'bank'}, { root: true }).then(() => {
          this.banks.data.unshift(this.$store.getters["admin/Getbank"]);
          this.display = false;
          this.$bvModal.hide("post-update-modal-bank");
          window.Toast.fire({
            icon: "success",
            title:'bank created successfuly',
          });
        })
        .catch((err) => {
          this.bank.errors.errors = err;
