            ...
            <template slot-scope="{row,index}">
              <tr  >
                ...
                <td style="vertical-align: middle">{{ row.price*1-row.credit-row.gift}}</td>
                  ...
                  <td style="vertical-align: middle"><b-button class="btn btn-success" @click="given(row.user.id)">Done</b-button></td>
              </tr>
            </template>
          </datatable>
          </div>
...
export default {
  data() {
    return {
      ...
      columns: [
        ...
        { label: "Total Amoount", field: "total" },
  ...
  methods: {
    getmyitem() {
      var api='admin_get_casher_reports'
    ...
    given(id) {
      var api='admin_given_casher_reports'
      this.$store.dispatch('admin/AdminController',{'data':{},'method':'get','api':api,'variable':'status','params':'/'+id}, { root: true }).then(() => {
        window.Toast.fire({
            icon: "success",
